...
(function() {
  var advance, bind, days, emit, enumerate, explain, hours, human, intervals, months, parse, primAdvance, soon, summarize,
    __slice = [].slice;
  ...
  parse = function(text) {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      try {
        ...
        } else if (months[word] || days[word] || hours[word]) {
  ...
  primAdvance = function(date, issue, count) {
    ...
    for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
      ...
      result = months[offset] ? new Date(y, months[offset] - 1, d, h) : days[offset] ? new Date(y, m, d + (7 - result.getDay() + days[offset] - 1) % 7, h) : hours[offset] ? new Date(y, m, d, h + 6 * (hours[offset] - 1)) : void 0;
    }
    return result;
  };
